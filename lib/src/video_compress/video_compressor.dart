...
import 'package:mb_video_compress/src/progress_callback/compress_mixin.dart';
...
IVideoCompress get MBVideoCompress => _VideoCompressImpl.instance;
...
extension Compress on IVideoCompress {
  ...
  /// ```
  Future<MBMediaInfo> getMediaInfo(String path) async {
    // Not to set the result as strong-mode so that it would have exception to
    // lead to the failure of compression
    final jsonStr = await (_invoke<String>('getMediaInfo', {'path': path}));
    final jsonMap = json.decode(jsonStr!);
    return MBMediaInfo.fromJson(jsonMap);
  }
  ...
  /// debugPrint(info.toJson());
  /// ```
  Future<MBMediaInfo?> compressVideo(
    String path, {
    MBVideoQuality quality = MBVideoQuality.DefaultQuality,
